...
public class miniparallel_rung2 : MonoBehaviour
{
    // Update is called once per frame
    void Update()
    {
        if (this.transform.childCount == 0 && this.transform.parent.GetChild(0).childCount == 0)
        {
        ...
        if (this.transform.childCount > this.transform.parent.GetChild(0).childCount)
        {
            for (int i = 0; i <= this.transform.childCount; i++)
            {
                ...
                if (this.transform.childCount == i)
                {
                    ...
                    {
                        RectTransform miniRectTransform2 = this.transform.parent.GetChild(0).GetComponent<RectTransform>();
